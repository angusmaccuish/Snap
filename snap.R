...
# Calculate theoretical probability of a pair occurring within the first k cards, with a rogue card.
# Hardcoded for case of suits=2 case.
# TODO - combine with vanilla case, as they are so close (as done for s=3)
pair.probability.with.1.rogue <- function(k, ranks=13) {
  f <- function(s) {
    p <- (-1)^(s+1)
    p <- p * choose(ranks, s)
    p <- p * 2^s
    p <- p * factorial(2*ranks-2*s) / factorial(2*ranks+1)
    p <- p * factorial(k-s) / factorial(k-2*s-1)
    return (p)
  }
  max = floor((k-1)/2)
  p <- if (max > 0) sum(sapply(1:max, f)) else 0
  weight <- k/(2*ranks+1)
  p <- p * weight + pair.probability(k, ranks) * (1-weight)
  return (p)
}
